...
describe('core/async/modules/timers', () => {
  ...
  [['timeout'], ['interval'], ['immediate'], ['idleCallback', 'requestIdleCallback', 'cancelIdleCallback']].forEach(([method, reg, clear]) => {
    ...
    const mute = `mute-${method}`.camelize(false),
          unmute = `un${mute}`;
    const suspend = `suspend-${method}`.camelize(false),
    ...
    it(`\`${method}\` with labels`, done => {
      const $a = new _async.default(),
            spy = jest.fn(),
            res = [];
      $a[reg](onResolve(res, 'first'), ...args, {
        label: $$.foo,
        onClear: onReject(spy)
      });
      $a[reg](onResolve(res, 'second'), ...args, {
        label: $$.foo,
        onClear: onReject(spy)
      });
      setTimeout(() => {
  ...
  ['sleep', 'nextTick', 'idle', 'wait'].forEach(method => {
    it(`\`${method}\``, done => {
      const $a = new _async.default(),
            args = [];
      let i = 0;
