...
describe('core/async/modules/proxy `promise`', () => {
  ...
  it('cancel promise by label', async () => {
    let i = 0;
    const label = 'label';
    $a.promise(new Promise(resolve => setTimeout(resolve, 100)), {
      label
    }).then(() => {
      i++;
    }).catch(() => undefined);
  ...
  describe('suspendPromise/unsuspendPromise`', () => {
    it('suspending a promise by id', async () => {
      let i = 0;
      const promise = $a.promise(new Promise(resolve => setTimeout(resolve, 150)));
      promise.then(() => {
        i++;
      });
      expect(i).toEqual(0);
    ...
    it('suspending a promise by a label', async () => {
      ...
      const promise = $a.promise(new Promise(resolve => setTimeout(resolve, 150)), label);
      promise.then(() => {
        i++;
      });
      expect(i).toEqual(0);
    ...
    it('suspending a promise by a group', async () => {
      ...
      const promise = $a.promise(new Promise(resolve => setTimeout(resolve, 150)), group);
      promise.then(() => {
        i++;
      });
      expect(i).toEqual(0);
