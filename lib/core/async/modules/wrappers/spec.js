...
describe('core/async/modules/wrappers', () => {
  ...
  describe('`wrapDataProvider`', () => {
    ...
    it('if a group is not provided should use a class name from the provider', () => {
      ...
      expect($a.request.calls.mostRecent().args[1]).toEqual({
    ...
    it('should provide a group into a nested event emitter wrapper and replace the original emitter with a wrapper', () => {
      ...
      expect($a.wrapEventEmitter.calls.mostRecent().args).toEqual([provider.emitter, {
        group: 'example'
      }]);
  ...
  describe('`wrapEventEmitter`', () => {
    ...
    it('normalizes of input parameters', () => {
      ...
      emitter.on('foo', () => null);
      expect($a.on.calls.mostRecent().args.slice(3)).toEqual([{}]); // [true] => [{}, true]
  ...
  describe('`wrapStorage`', () => {
    ...
    function testMethod(methodName) {
      describe(`\`${methodName}\``, () => {
        ...
        it('should mark a storage by the global group', async () => {
          ...
          await promise;
          expect(spyPromise).toHaveBeenCalledOnceWith({
            type: 'clearAsync',
            reason: 'group'
          });
        });
        ...
        it('should concatenate global and local groups', async () => {
          const wrappedStorage = $a.wrapStorage(mockedStorage, {
            group: 'bla'
                ...
                spyWithoutLocal = jest.fn(),
                spyWithLocal = jest.fn();
          ...
          expect(spyWithLocal).toHaveBeenCalledOnceWith({
            type: 'clearAsync',
            reason: 'group'
          });
        });
        it('should separate async options from additional parameters', () => {
          ...
          expect(mockedStorage[methodName].calls.mostRecent().args[2]).toEqual({
            additionalArg: true
          });
          expect(mockedStorage[methodName].calls.mostRecent().args[3]).toEqual({
            notAsyncOption: true
          });
        });
      });
    ...
    describe('`namespace`', () => {
      ...
      it('should concatenate global and local groups', () => {
        ...
        expect($a.wrapStorage.calls.mostRecent().args[1]).toEqual({
          group: 'bla:foo'
        });
      });
    });
