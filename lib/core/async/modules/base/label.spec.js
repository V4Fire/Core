...
describe('core/async/modules/base `label`', () => {
  ...
  it('label collision', done => {
    const $a = new _async.default(),
      spy = jest.fn(),
      res = [];
    $a.setTimeout(onResolve(res, 'first'), 10, {
      label: $$.foo,
      onClear: onReject(spy)
    });
    $a.setTimeout(onResolve(res, 'second'), 10, {
      label: $$.foo,
      onClear: onReject(spy)
    });
    setTimeout(() => {
  ...
  it('label collision with promises', done => {
    const $a = new _async.default(),
      spy = jest.fn(),
      res = [];
    $a.promise(Promise.resolve('first'), {
      label: $$.foo
    }).then(onResolve(res), onReject(spy));
    $a.promise(Promise.resolve('second'), {
      label: $$.foo
    }).then(onResolve(res), onReject(spy));
    setTimeout(() => {
      expect(res).toEqual(['second']);
