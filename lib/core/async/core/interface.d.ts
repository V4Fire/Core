...
export declare type Label = string | symbol;
export declare type Join = boolean | 'replace';
/**
 * Values by which a task can be marked
 */
export declare type Marker = 'muted' | '!muted' | 'paused' | '!paused';
/**
 * The reason why a task can be marked
 */
export declare type MarkReason = 'id' | 'label' | 'group' | 'rgxp' | 'all';
/**
 * The reason why a task can be killed (cleared)
 */
export declare type ClearReason = MarkReason | 'muting' | 'collision';
...
export declare type ProxyCb<A = unknown, R = unknown, CTX extends object = Async> = A extends never ? ((this: CTX) => R) : A extends any[] ? ((this: CTX, ...args: A) => R) : ((this: CTX, e: A) => R) | Function;
export declare type AsyncCb<CTX extends object = Async> = ProxyCb<TaskCtx<CTX>, void, CTX>;
...
export declare type TaskNamespaces = Namespaces | PrimitiveNamespaces | PromiseNamespaces;
...
export declare type StrictClearOptions = Omit<ClearOptions, 'label'> | Overwrite<ClearOptions, {
    label: Label;
    group: Group | RegExp;
}>;
export declare type StrictClearOptionsId<ID = any> = Omit<ClearOptionsId<ID>, 'label'> | Overwrite<ClearOptionsId<ID>, {
    label: Label;
    group: Group | RegExp;
}>;
...
export declare type TaskCtx<CTX extends object = Async> = {
    /**
     * The task type
     */
    type: string;
