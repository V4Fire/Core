  ...
  describe('timer method', () => {
    ...
    describe('`finish`', () => {
      ...
      it('sends additional data along with a delta', () => {
        ...
        expect(testEngine.sendDelta).toHaveBeenCalledOnceWith('manual.metrics', 1, {
          name: 'secret'
        });
      });
    ...
    describe('`markTimestamp`', () => {
      ...
      it('sends additional data along with a delta', () => {
        ...
        expect(testEngine.sendDelta).toHaveBeenCalledOnceWith('manual.mark', 1, {
          title: 'none'
        });
      });
    ...
    describe('`namespace`', () => {
      ...
      it('creates another timer instance which `markTimestamp` method does not affect the same method of the previous timer', () => {
        ...
        expect(testEngine.sendDelta.calls.allArgs()).toEqual([['network.v1.attach-headers', 1, undefined], ['network.attach-headers', 2, undefined]]);
      });
      ...
      it('creates another timer instance which `start`/`finish` methods do not affect the same methods of the previous timer', () => {
        ...
        expect(testEngine.sendDelta.calls.allArgs()).toEqual([['network.auth.login', 1, undefined], ['network.login', 2, undefined]]);
      });
      it('does not send a delta', () => {
        const runner = new _impl.PerfTimersRunner(testEngine);
