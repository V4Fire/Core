...
describe('core/request', () => {
  ...
  engines.forEach((engine, name) => {
    describe(`with the "${name}" engine`, () => {
      ...
      it('json `get`', async () => {
        expect((await request('http://localhost:3000/json/1')).data).toEqual({
          id: 1,
          value: 'things'
        });
      });
      it('json `get` with caching', async () => {
        const url = 'http://localhost:3000/json/1',
              get = request({
      ...
      it('text/xml `get`', async () => {
        expect((await request('http://localhost:3000/xml/text')).data.querySelector('foo').textContent).toBe('Hello world');
      });
      it('application/xml `get`', async () => {
        expect((await request('http://localhost:3000/xml/app')).data.querySelector('foo').textContent).toBe('Hello world');
      });
      it('xml `get` with a query', async () => {
        const {
          data
        } = await request('http://localhost:3000/search', {
          query: {
            q: 'bla'
          }
        });
      ...
      it('json `put` with headers', async () => {
        ...
        expect(req.data).toBe('{"message": "Success"}');
        expect(req.response.getHeader('Content-Type')).toBeUndefined();
      ...
      it('request factory with rewriting of URL', async () => {
        const resolver = () => ['http://localhost:3000', 'json', 1],
      ...
      it('request with a low timeout', async () => {
        ...
        try {
          await request('http://localhost:3000/delayed', {
            timeout: 100
          });
        } catch (e) {
          err = e;
      ...
      it('request with a high timeout', async () => {
        const req = await request('http://localhost:3000/delayed', {
          timeout: 500
        });
        expect(req.response.ok).toBeTrue();
      });
      describe('responses with no message body', () => {
        for (const status of emptyBodyStatuses) {
          it(`response with ${status} status`, async () => {
            const req = await request(`http://localhost:3000/octet/${status}`, {
              okStatuses: status
            });
      ...
      it('failing after given attempts', async () => {
        try {
          await request('http://localhost:3000/retry/bad', {
            retry: 3
          });
        } catch (err) {
