...
describe('core/request', () => {
  const engines = new Map([['fetch', _fetch.default], ['xhr', _xhr.default], ['provider', (0, _provider.default)('Provider')], ['chain provider', (0, _provider.default)(TestRequestChainProvider)]]);
  ...
  engines.forEach((engine, name) => {
    describe(`with the "${name}" engine`, () => {
      ...
      it('failing after the given attempts', async () => {
        ...
        const body = await err.details.deref().response.json();
        expect(err).toBeInstanceOf(_request.RequestError);
        expect(err.type).toBe(_request.RequestError.InvalidStatus);
      ...
      if (name === 'xhr') {
        ...
        it('parsing JSON from a stream', async () => {
          const req = request('http://localhost:4000/json/users', {
            streamDecoder: data => (0, _combinators.seq)((0, _stream.assemble)((0, _stream.pick)(data, 'total')), (0, _stream.streamArray)((0, _stream.andPick)(data, 'data')))
          });
          const res = [];
