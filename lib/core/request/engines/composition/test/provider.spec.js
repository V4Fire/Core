...
describe('core/request/engines/provider/compositor', () => {
  ...
  it('should invoke the decoder with correct data', async () => {
    ...
    const request1 = (opts, params, {
        boundRequest
      }) => boundRequest(new TestProviderDecoder1()).get({
        query: 1
      }),
      request2 = (opts, params, {
        boundRequest
      }) => boundRequest(new TestProviderDecoder2()).get({
        notQuery: 2
      });
  ...
  it('should clear the cache for all providers', async () => {
    ...
    const request1 = (opts, params, {
      ...
      }) => boundRequest(provider1).get(),
      request2 = (opts, params, {
        boundRequest: boundRequest
      }) => boundRequest(provider2).get();
  ...
  it('should provide the constructor options of the Provider to the request', async () => {
    ...
    let providerCtorArgs1, providerCtorArgs2;
  ...
  it('should properly cache provider instances', async () => {
    ...
    let TestProviderDecoder1 = (0, _data.provider)(_class13 = class TestProviderDecoder1 extends _data.default {
      static request = _data.default.request({
        cacheStrategy: 'queue'
      });
      baseURL = 'http://localhost:5000/json/1';
    }) || _class13;
    let TestProviderDecoder2 = (0, _data.provider)(_class14 = class TestProviderDecoder2 extends _data.default {
      static request = _data.default.request({
        cacheStrategy: 'queue'
      });
      baseURL = 'http://localhost:5000/json/2';
