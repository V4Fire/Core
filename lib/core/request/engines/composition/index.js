  ...
  const engine = (requestOptions, params) => {
    ...
    return new _abortable.default((resolve, reject) => {
      ...
      const promises = compositionRequests.map(r => _structures.SyncPromise.resolve(r.requestFilter?.(options)).then(filterValue => {
        ...
        return r.request(options).then(boundRequest.bind(null, async)).then(async request => {
      ...
      gatherDataFromRequests(promises, options).then(({
        data,
        status,
        headers
      }) => {
        resolve(new _request.Response(data, {
          ...
          status: status ?? _statusCodes.default.OK,
          headers: headers ?? {},
...
function accumulateData(accumulator, newData, compositionRequest) {
  const {
      as
    } = compositionRequest,
    {
