...
function createProviderEngine(src, methodsMapping = {}) {
  ...
  function dataProviderEngine(params) {
    const p = Object.select(params, _const.availableParams),
    ...
    methodsMapping = { ...defaultRequestMethods,
      ...methodsMapping
    };
    const parent = new _abortable.default(async (resolve, reject, onAbort) => {
      ...
      let body = _data.queryMethods[requestMethod] === true ? p.query : p.body;
      let urlProperty = `base-${providerMethod}-URL`.camelize(false);

      if (!Object.isTruly(provider[urlProperty])) {
        urlProperty = _data.urlProperties[0];
      }

      const providerToRequest = isSimpleRequest ? createMixedProvider(provider, {
        [urlProperty]: p.url.replace(_data.globalOpts.api ?? '', '')
      }) : provider;
      ...
      const headers = Object.reject(res.headers, 'content-type');
      return resolve(new _response.default(data, {
        responseType: 'object',
