...
class Filter extends _abstractFilter.default {
  ...
  *finishTokenProcessing() {
    ...
    let commonLength = 0; // Find the common part

    for (const n = Math.min(stackLength, objStackLength); commonLength < n && stack[commonLength] === objStack[commonLength]; commonLength++) {} // Close old objects
