...
function watch(obj, path, handler, handlers, opts, root, top) {
  ...
  proxy = new Proxy(unwrappedObj, {
    get: (target, key) => {
      ...
      if (Object.isSymbol(key) || blackListStore.has(key)) {
        ...
        if (isArray && (0, _helpers.isValueCanBeArrayIndex)(key)) {
          ...
          normalizedKey = key;
          const desc = Reflect.getOwnPropertyDescriptor(target, key); // Readonly non-configurable values can't be wrapped due Proxy API limitations
    ...
    defineProperty: (target, key, desc) => {
      ...
      const res = Reflect.defineProperty(target, key, mergedDesc);

      if (res) {
        if (val !== undefined) {
          Object.cast(proxy)[key] = val;
