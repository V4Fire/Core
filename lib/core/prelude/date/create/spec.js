...
describe('core/prelude/date/create', () => {
  describe('`create`', () => {
    ...
    it('new date based on templates: xxxx[.-/]xx[.-/]xx | xx[.-/]xx[.-/]xxxx', () => {
      const chunks = [today.getFullYear(), (today.getMonth() + 1).toString().padStart(2, 0), today.getDate().toString().padStart(2, 0)];
    ...
    it('new date based on templates without leading zeros', () => {
      const dates = [{
        date: '25.3.1989',
        withZeroes: '25.03.1989'
      }, {
        date: '3.03.1989',
        withZeroes: '03.03.1989'
      }, {
        date: '1989.3.25',
        withZeroes: '1989.03.25'
      }, {
        date: '1989.03.3',
        withZeroes: '1989.03.03'
      }];
      dates.forEach(({
        date,
        withZeroes
        ...
        expect(Date.create(replaceDot(date, '-'))).toEqual(Date.create(replaceDot(withZeroes, '-')));
        expect(Date.create(replaceDot(date, '/'))).toEqual(Date.create(replaceDot(withZeroes, '/')));
      });
    });
    ...
    it('new date based on non-standard templates', () => {
      const date = new Date(2015, 9, 11, 10, 0, 0),
