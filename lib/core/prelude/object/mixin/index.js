...
(0, _extend.default)(Object, 'mixin', function mixin(opts, target, ...objects) {
  ...
  let resolvedTarget,
      type = (0, _helpers.getType)(target);

  if (type === '') {
    for (let i = 0; i < objects.length; i++) {
      type = (0, _helpers.getType)(objects[i]);
  ...
  const canUseNativeAssign = !p.deep && dataIsSimple && !skipUndefs && !concatArrays && !onlyAccessors && !withDescriptors && !Object.isTruly(onlyNew) && !p.withProto && !p.withNonEnumerables && !p.extendFilter && !p.filter;
  ...
  for (let i = 0; i < objects.length; i++) {
    ...
    const isSimple = simpleTypes[(0, _helpers.getType)(extender)] === true;
    Object.forEach(extender, (el, key) => {
      if (p.filter && !Object.isTruly(p.filter(el, key, extender))) {
        return;
      }

      const propDesc = withDescriptors || onlyAccessors ? Object.cast(el) : null;
      const needExtendDescriptor = dataIsSimple && isSimple && (withDescriptors || onlyAccessors && propDesc != null && (propDesc.get != null || propDesc.set != null));
      ...
      const extVal = isSimple ? extender[key] : el;

      if (extVal === resolvedTarget || extVal === extender) {
        return;
      }

      let canDeepExtend = Object.isTruly(extVal);
      ...
      if (canDeepExtend) {
        ...
        struct = valIsArray ? [] : (0, _helpers.getSameAs)(extVal);
        canDeepExtend = struct != null;
      }

      if (p.deep && canDeepExtend) {
        const canExtendSrcProto = p.withProto && dataIsSimple && (0, _helpers.canExtendProto)(oldVal);
        let srcIsArray = Object.isArray(oldVal);
        ...
        if (valIsArray) {
          ...
          if (srcIsArray) {
            if (concatArrays) {
              ...
              resolvedTarget[key] = Object.isFunction(concatFn) ? concatFn(old, extVal, key) : old.concat(extVal);
              return;
            }
