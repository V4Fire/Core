...
describe('core/range', () => {
  ...
  it('infinity ranges', () => {
    expect(new _range.default().span()).toBePositiveInfinity();
    expect(new _range.default().toString()).toBe('..');
    expect(new _range.default().contains(-10)).toBeTrue();
    expect(() => new _range.default().toArray()).toThrowError("Can't create an array of the infinitive range. Use an iterator instead.");
    expect(new _range.default(0).span()).toBePositiveInfinity();
    expect(new _range.default(0).toString()).toBe('0..');
    expect(new _range.default(0).contains(-10)).toBeFalse();
    expect(new _range.default(0).contains(10)).toBeTrue();
    expect(new _range.default(0).contains(0)).toBeTrue();
    expect(new _range.default([0]).contains(0)).toBeFalse();
    expect(new _range.default(null, 0).span()).toBePositiveInfinity();
    expect(new _range.default(-Infinity, 0).toString()).toBe('..0');
    expect(new _range.default(null, 0).contains(-10)).toBeTrue();
    expect(new _range.default(null, 0).contains(10)).toBeFalse();
    expect(new _range.default(null, 0).contains(0)).toBeTrue();
    expect(new _range.default(null, [0]).contains(0)).toBeFalse();
    expect(new _range.default(new Date()).span()).toBePositiveInfinity();
    expect(new _range.default('a').span()).toBePositiveInfinity();
    expect(new _range.default('a').toString()).toBe('a..');
    expect(new _range.default('a').contains('0')).toBeFalse();
    expect(new _range.default('a').contains('c')).toBeTrue();
    expect(new _range.default('a').contains('a')).toBeTrue();
    expect(new _range.default(['a']).contains('a')).toBeFalse();
    expect(new _range.default(null, 'a').span()).toBePositiveInfinity();
    expect(new _range.default(-Infinity, 'a').toString()).toBe('..a');
    expect(new _range.default(null, 'a').contains('0')).toBeTrue();
    expect(new _range.default(null, 'a').contains('c')).toBeFalse();
    expect(new _range.default(null, 'a').contains('a')).toBeTrue();
    expect(new _range.default(null, ['a']).contains('a')).toBeFalse();
  });
  it('`clamp`', () => {
  ...
  it('checking of containment', () => {
    expect(new _range.default(0, 3).contains(1)).toBeTrue();
    expect(new _range.default(0, 3).contains(10)).toBeFalse();
    expect(new _range.default(0).contains(10)).toBeTrue();
    expect(new _range.default(1, 10).contains(new _range.default(4, 6))).toBeTrue();
    expect(new _range.default(1, 10).contains(new _range.default(4, 12))).toBeFalse();
    expect(new _range.default(1).contains(new _range.default(4, 6))).toBeTrue();
    expect(new _range.default(1, 10).contains(new _range.default(4))).toBeFalse();
    expect(new _range.default(1, 10).contains(new _range.default('a', 'z'))).toBeFalse();
    expect(new _range.default('a', 'd').contains('b')).toBeTrue();
    expect(new _range.default('a', 'd').contains('z')).toBeFalse();
  });
  it('`intersection`', () => {
  ...
  it('`isValid`', () => {
    expect(new _range.default(0, 2).isValid()).toBeTrue();
    expect(new _range.default(0, '2').isValid()).toBeTrue();
    expect(new _range.default(0, 'a').isValid()).toBeTrue();
    expect(new _range.default(new Date(), new Date('foo')).isValid()).toBeFalse();
  });
  it('`toString`', () => {
