...
describe('core/prelude/number/format', () => {
	it('`pad`', () => {
		...
		expect((10).pad(3, {sign: true})).toBe('+010');
		expect((10).pad({length: 3, sign: true})).toBe('+010');
	...
	it('`format`', () => {
		...
		expect(100.50.format({style: 'currency', currency: 'USD'}, 'en-us'))
